...
| **ROI** | [`business_metrics.py`](file:///c:/Users/Dlameros/Desktop/rule_based/business_metrics.py) | **Business ROI Engine:** Расчет экономии ФОТ и отбора вызовов. | ROI консольный отчет |

---

## 📈 Фактические Метрики (Бенчмарк `318_test.csv`)

Строгие экспериментальные показатели на 318 размечанных диалогах бенчмарка:

| Метрика | Робот «Ольга» (Baseline) | Pure Rule-Based Auditor | Фактический прирост |
|---|---|---|---|
| **Accuracy (Точность)** | **46.86%** (149/318) | **47.80%** (152/318) | **+0.94%** |
| **Macro F1-Score** | **0.3854** | **0.4581** | **+0.0726 (+18.9% рост качества)** |
| **Precision (Отток - Status 2)** | **30.6%** | **35.7%** | **+5.1% (снижение ложных вызовов)** |
| **Recall (Менеджер - Status 4)**| **12.0%** (10/83) | **49.4%** (41/83) | **+37.4% (рост улавливания запросов к менеджеру в 4.1 раза)** |
| **Recall (Лояльные/SIM - Status 3)**| **26.0%** (13/50) | **74.0%** (37/50) | **+48.0% (рост распознавания остающихся в 2.8 раза)** |

---

## 💼 Фактический Операционный Эффект (Отбор из 3 200 диалогов под лимит 3 000)

При отборе из **3 200 входящих диалогов «серой зоны»** с учетом физического лимита колл-центра в **3 000 звонков**:

| Операционный показатель | Фактическое значение | Описание |
|---|---|---|
| **Отбор целевых звонков (S2 + S4)** | **1 419 звонков (44.3%)** | **100% входит в лимит 3 000** (задействовано лишь 47.3% мощности операторов) |
| **Автономный отсев правилами (S1 + S3)**| **1 781 звонок (55.7%)** | **0 ₽ затрат на API**, операторы не делают бесполезных вызовов |
| **Экономия времени операторов** | **148.4 чел-часов / мес** | Сбереженное время от непроизводительных вызовов |
| **Прямая экономия на ФОТ** | **66 788 ₽ / мес (~801 450 ₽ / год)** | Фактическая экономия операционного бюджета при 450 ₽/час |
| **Затраты на нейросети / API** | **0.00 ₽** | **100% Локальный детерминированный Python-скрипт** |
...
* **Evaluation Engine:** [`eval_pipeline.py`](file:///c:/Users/Dlameros/Desktop/rule_based/eval_pipeline.py), [`eval_comparison.py`](file:///c:/Users/Dlameros/Desktop/rule_based/eval_comparison.py), [`business_metrics.py`](file:///c:/Users/Dlameros/Desktop/rule_based/business_metrics.py)
